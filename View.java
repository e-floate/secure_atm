...
class View
{
    ...
    public void start(Stage window)
    {
        ...
        // controls
        title  = new Label("Test");           // Message bar at the top for the title
        grid.add( title, 0, 0);         // Add to GUI at the top       

        message  = new TextField("Test");     // text field for numbers and error messages 
        message.setEditable(false);     // Read only (user can't type in)
        grid.add( message, 0, 1);       // Add to GUI on second row                      

        reply  = new TextArea("Test");        // multi-line text area for instructions
        ...
        // the css file
        String labels[][] = {
                {"7",    "8",  "9",  "",  "Dep",  "PW"}, //Pass is password change
                {"4",    "5",  "6",  "",  "W/D",  ""},
                {"1",    "2",  "3",  "",  "Bal",  "Fin"},
                {"CLR",  "0",  "",   "",  "",     "Ent"} };
